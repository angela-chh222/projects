...
let firstNames = students.map(student => {
	return student.fname;
	})

let filteredNames = students.filter(student => student.egrade >= 80);
let fullNames = filteredNames.map(student => {
	allFullNames = student.fname + " " + student.lname;
	return allFullNames;
});

let aGradeArray = students.map(student => {
	return student.agrade;
	})
let tGradeArray = students.map(student => {
	return student.tgrade;
	})
let eGradeArray = students.map(student => {
	return student.egrade;
	})

let total = 0;

//let sum2 = aGradeArray.reduce((total, amount) => total + amount);
function sum(gradeArray) {
	let total = gradeArray.reduce((total, amount) => total + amount);
	return total;
}
let aveGrade = ((sum(aGradeArray)*0.4) + (sum(tGradeArray)*0.1) + (sum(eGradeArray)*0.5))/students.length;
